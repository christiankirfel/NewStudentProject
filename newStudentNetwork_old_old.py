...
class neuralNetworkEnvironment(object):
    ...
    def __init__(self, changing_parameter, epoch):
        ...
        #= SGD(lr = 0.005, momentum = 0.001,  nesterov=False)
        #keras.optimizers.adam(lr=0.001, beta_1=0.9, beta_2=0.999, epsilon=1e-8, decay = 1e-6, clipnorm = 0, clipvalue = 0.005)  #, amsgrad = FALSE
...
#        plt.legend(loc="upper right", prop={'size' : 7})
    ...
    def plotloss_multi(self, iterations):
        ...
        plt.gcf().clear()
5
#In the following options and variables ar5e read in
#This is done to keep the most important f5eatures clearly represented

#You need to find a nice set of variables to use and add them as a text file
#with open('/cephfs/user/s6chkirf/whk_ANN_va riables.txt','r') as varfile:
...
#    OptionDict = dict()#Loading the packages for running the networks
import os
import keras
import math
import sys
...
#first_training.plotSeparatiLayers
#first_training.plotAccuracyLayers
#first_training.plotLosses()Layers

"""
first_training = neuralNetworkEnvironment(2, 20)
first_training.initialize_sample()
first_training.buildDiscriminator()
first_training.trainDiscriminator()
first_training.predictModel()
