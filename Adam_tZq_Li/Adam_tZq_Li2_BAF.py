...
output_path = 'output/' + sys.argv[11] + '_final/' + sys.argv[12] + '/Adam_' + sys.argv[1] + '_Lr_' + sys.argv[2] + '_Nodes_' + sys.argv[3] + '_Layers_' + sys.argv[4] + '_Dropout_' + sys.argv[5]
...
#Additionally the last layer is saved to be an optional input to the adversary
    ...
    def plotSeparation(self):
        ...
        self.signal_histo_val = []
        self.background_histo_val = []
        for i in range(len(self.sample_training)):
            if self.target_training[i] == 1:
                self.signal_histo_test.append(self.model_prediction_training[i])
            if self.target_training[i] == 0:
                self.background_histo_test.append(self.model_prediction_training[i])
        for i in range(len(self.sample_validation)):
            if self.target_validation[i] == 1:
                self.signal_histo_val.append(self.model_prediction_test[i])
            if self.target_validation[i] == 0:
                self.background_histo_val.append(self.model_prediction_test[i])           
        plt.hist(self.signal_histo_test, range=[0., 1.], linewidth = 2, bins=30, histtype="step", density = True, color="red", label = "Signal_Test")
        plt.hist(self.background_histo_test, range=[0., 1.], linewidth = 2, bins=30, histtype="step", density = True, color="green", label = "Background_Test")
        plt.hist(self.signal_histo_val, range=[0., 1.], linewidth = 2, bins=30, histtype="step", density = True, color="blue", label = "Signal_Validation")
        plt.hist(self.background_histo_val, range=[0., 1.], linewidth = 2, bins=30, histtype="step", density = True, color="yellow", label = "Background_Validation")
        plt.legend()
