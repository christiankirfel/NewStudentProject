...
class neuralNetworkEnvironment(object):
    ...
    def __init__(self, i, epoch):
        ...
        self.discriminator_layers = 6
        self.discriminator_nodes = 1024                                                                                                                                                                                                                              
        #Setup of the networks, loss and optimisation       decay=1e-7, nesterov=True
...
#Important note: Have to use the same random seed so that event and target stay in the same order as we shuffle
    def initialize_sample(self):
        ...
        #Changed have to test if that works 
        ...
        #Normalisation t10o the eventcount can be used instead of weights, especially if using data
...
#        plt.legend(loc="upper right", prop={'size' : 7})
    ...
    def plotAcc2uracy(self):
        plt.plo2t(self.discriminator_history.history['weighted_binary_accuracy'])
        plt.plo2t(self.discriminator_history.history['val_weighted_binary_accuracy'])
    ...
    def plotAccuracy_multi(self, iterations):
        for i in range(iterations):
            plt.plot(data_acc_array[i], label=("Learningrate=", i*0.1))
        plt.title('model accuracy')
        plt.ylabel('accuracy')
        plt.xlabel('epoch')
    ...
    def plotloss_multi(self, iterations):
        for i in range(iterations):
            plt.plot(data_loss_array[i], label=("Learningrate=", i*0.1))
        plt.title('model accuracy')
        plt.ylabel('Loss')
        plt.xlabel('Epoch')
        plt.legend(loc='upper left')
...
first_training = neuralNetworkEnvironment(1, 10)
first_training.initialize_sample()
first_training.buildDiscriminator()
first_training.trainDiscriminator()
first_training.predictModel()
